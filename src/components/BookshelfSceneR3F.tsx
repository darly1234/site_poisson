// Implementação fiel ao código original — Three.js/R3F com capas reais da API
...
function BookMesh({ position, rotation, index, scrollRef, tex, spineColor }:
  ...
  useFrame(({ pointer, clock }) => {
    ...
    // float integrado (sem Float component extra)
    ref.current.position.x = position[0] + Math.cos(t * 0.55 + index * 0.6) * 0.07;
    ref.current.position.y = position[1] + Math.sin(t * 0.75 + index * 0.7) * 0.13;
...
function Field({ scrollRef, covers }: { scrollRef: React.MutableRefObject<number>; covers: string[] }) {
  ...
  const books = useMemo(() => {
    ...
    const COUNT = 120, MAX_DEPTH = 14;
    // Limita texturas únicas a 20 — useTexture faz cache por URL, então
    // books 20+ reutilizam as mesmas texturas sem custo extra de GPU
    const MAX_TEX = Math.min(covers.length, 20);
    for (let i = 0; i < COUNT; i++) {
      const z = -(i / COUNT) * MAX_DEPTH - 0.4;
      const spread = Math.max(viewport.width * 1.9, 18);
      // 5 linhas simétricas (-2,-1,0,1,2) e 8 colunas simétricas (-3.5..3.5)
      // 120 livros ÷ 5 linhas = 24 grupos ÷ 8 cols = 3 grupos por coluna → distribuição perfeita
      const row = (i % 5) - 2;
      const col = (Math.floor(i / 5) % 8) - 3.5;
      const jx = (rng(i + 1) - 0.5) * spread * 0.50;
      const jy = (rng(i + 7) - 0.5) * 3.8;
      arr.push({
        p: [col * (spread / 8) + jx, row * 1.6 + jy, z],
        r: [(rng(i + 3) - 0.5) * 0.45, (rng(i + 5) - 0.5) * 1.0, (rng(i + 11) - 0.5) * 0.22],
        i,
        t: MAX_TEX > 0 ? covers[i % MAX_TEX] : undefined,
      });
    }
    return arr;
...
export default function BookshelfSceneR3F() {
  ...
  useEffect(() => {
    fetch(API_URL)
      ...
      .then((data: Array<{ capa?: string; cover?: string }>) => {
        // &w=200 reduz memória de textura ~70% vs &w=360
        const urls = data
          .map(b => (b.capa || b.cover || "").replace(/&w=\d+/, "&w=200"))
          .filter(Boolean);
